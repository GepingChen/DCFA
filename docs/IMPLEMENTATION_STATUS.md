...
bounded managed TabPFN Client path now exercises one fixed smoke request and the
three website-demo presets through the typed agent runtime, but it is development
mechanics rather than a locked Track T run. Publishable Track T, real-data Track
T/H, and live-model Track A remain explicitly blocked on the external inputs
listed below.
...
| Managed TabPFN demo | Exact `tabpfn-client==0.3.3` profile; fixed/bounded synthetic data; typed smoke and website routes; batched prediction calls; observed service package/trace metadata; transport-safe bar-distribution CDF; evidence-linked immutable artifacts | `development_only`; no arbitrary upload or sklearn fallback; service checkpoint/image hashes are unavailable, so this cannot enter locked Track T |
...
| Public UI | Lazy Gradio Blocks app, managed TabPFN-only execution, no Hillstrom/Torch/client import before a run, atomically reserved immutable run directories, bounded queue and controls, evidence-linked values/plot, mobile layout, liveness/credential-aware readiness endpoints, non-root container and local Compose profile | Development workflow demonstration only; synthetic inputs leave the machine; no public HTTPS deployment and not release-eligible |
...
.venv/bin/python -m pytest                             88 passed in 35.54s
...
website demo integration tests                        10 passed; all managed paths + service
...
Docker image/container                                 managed lock; non-root 10001:10001; healthy
